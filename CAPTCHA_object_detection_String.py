...
def Captcha_detection(image,average_distance_error=3):
    with detection_graph.as_default():
        with tf.compat.v1.Session(graph=detection_graph) as sess:
            ...
            # print('Original Dimensions : ',image_np.shape)
            ...
            # print('Resized Dimensions : ',image_np.shape)
...
##            cv2.imshow(title, cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB))
            ...
            #Captcha_detection("image-67.png")    
                ...
                for Symbol in range(11):
                    if classes[0][i] == Symbol: # check if detected class equal to our symbols
                        if scores[0][i] >= 0.65: # do something only if detected score more han 0.65
                            ...
                            captcha_array.append([category_index[Symbol].get('name'), round(mid_x,3), round(float(scores[0][i]*100),2)])
##            print(captcha_array)
...
##print(Captcha_detection("image-68.png"))
